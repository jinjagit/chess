...
UI: Indicator shows side to move. Total pieces value (and difference) for each side updates when piece taken / pawn promoted. Game status info shown in box to right of board. Board can be flipped at any point in game (including when promotion menu displayed). Button hover effects and tool-tips working smoothly.
